...
def get_events():
    sql = """SELECT events.id, events.user_id, events.title, events.event_date, users.username 
             FROM events 
...
def get_event(event_id):
    sql = """SELECT events.id, events.user_id, events.title, events.description, events.event_date, users.username 
             FROM events 
             JOIN users ON events.user_id = users.id 
             WHERE events.id = ?"""
...
def get_rsvps(event_id):
    sql = """SELECT users.username, rsvps.status 
             FROM rsvps 
